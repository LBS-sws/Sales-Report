...
class ReportfollowList extends CListPageModel
{
	...
	protected function getSQL($type) {
		...
		$sql = $type==1
			? "select j.*,b.name city_name,b.code,s.ServiceName, t1.StaffName as Staff01Name, t2.StaffName as Staff02Name, t3.StaffName as Staff03Name, j.SType as ServiceType  
				from followuporder as j left join officecity as o on j.City=o.City  
				left join enums as e on e.EnumID=o.Office 
				left join service as s on s.ServiceType=j.SType 
		...
		if (count($records) > 0) {
			foreach ($records as $k=>$record) {
				$this->attr[] = array(
                    ...
                    'StartTime'=>$record['JobDate'].' '.$record['StartTime'],
                    'FinishTime'=>$record['JobDate'].' '.$record['FinishTime'],
				);
			}
		}
