 ...
 */
class ReportfollowForm extends CFormModel
{
	/* User Fields */
    public $city = [];
    public $job = [];
...
//			array('code','safe','on'=>'edit'),
    ...
    public function batchdown($dates)
    {
        ...
        if ($jobids_data) {
            ...
            for ($fj = 0; $fj < count($jobids_data); $fj++) {
                $index = $jobids_data[$fj]['FollowUpID'];
                $sql_basic = "select j.FollowUpID,j.CustomerName,j.Addr,j.ContactName,j.Mobile,j.JobDate,j.StartTime,j.FinishTime,u.StaffName as Staff01,uo.StaffName as Staff02,ut.StaffName as Staff03,s.ServiceName,j.Status,j.City,j.ServiceType,j.FirstJob,j.FinishDate  from followuporder as j left join service as s on s.ServiceType=j.ServiceType left join staff as u on u.StaffID=j.Staff01 left join staff as uo on uo.StaffID=j.Staff02 left join staff as ut on ut.StaffID=j.Staff03 where j.JobID=" . $index;
                $this->basic = Yii::app()->db->createCommand($sql_basic)->queryRow();
                $sql_job = "select * from followuporder where FollowUpID=" . $index;
    ...
    public function batchcreate($dates)
    {
        ...
        if($city_off){
            ...
            $jobids = "select FollowUpID from followuporder where Status=3 and JobDate>='" . $dates['start_dt'] . "' and JobDate<='" . $dates['end_dt'] . "'";
        }
		if (!empty($dates['customer_name'])) $jobids .= " and CustomerName like '%".$dates['customer_name']."%'";
        $jobids_data = Yii::app()->db->createCommand($jobids)->queryAll();
        if ($jobids_data) {
            for ($fj = 0; $fj < count($jobids_data); $fj++) {
                $index = $jobids_data[$fj]['FollowUpID'];
                $sql_basic = "select j.FollowUpID as JobID,j.CustomerName,j.Addr,j.ContactName,j.Mobile,j.JobDate,j.StartTime,j.FinishTime,u.StaffName as Staff01,uo.StaffName as Staff02,ut.StaffName as Staff03,s.ServiceName,j.Status,j.City,j.SType as ServiceType  from followuporder as j left join service as s on s.ServiceType=j.SType left join staff as u on u.StaffID=j.Staff01 left join staff as uo on uo.StaffID=j.Staff02 left join staff as ut on ut.StaffID=j.Staff03 where j.FollowUpID=" . $index;
                $this->basic = Yii::app()->db->createCommand($sql_basic)->queryRow();
                //判断是否存在PDF
                $reportfile = Yii::app()->basePath . "/images/report/" . $city . "/" . $this->basic['JobDate'] . "/" . $index . '.pdf';
                if (!file_exists($reportfile)) {
...
//            没有查询到图片
                        ...
                        $employee01_signature = str_replace("data:image/jpg;base64,", "", isset($autograph['employee01_signature']) ? $autograph['employee01_signature'] : '');
                        $employee02_signature = str_replace("data:image/jpg;base64,", "", isset($autograph['employee02_signature']) ? $autograph['employee02_signature'] : '');
                        $employee03_signature = str_replace("data:image/jpg;base64,", "", isset($autograph['employee03_signature']) ? $autograph['employee03_signature'] : '');
                        //图片路径
                        ...
                        if (isset($autograph['customer_signature']) && $autograph['customer_signature'] != '' && $autograph['customer_signature'] != 'undefined') {
                            $cimageName = "lbs_" . date("His", time()) . "_" . rand(111, 999) . '.png';
                            $cimageSrc = $path . "/" . $cimageName;
